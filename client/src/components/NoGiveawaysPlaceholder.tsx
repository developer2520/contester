...
const GiveawayTutorial = () => {
  ...
  useEffect(() => {
    // Check if this is the first time the user is seeing the tutorial
    const hasSeenTutorial = localStorage.getItem('hasSeenTutorial');
    
    if (hasSeenTutorial) {
      setIsFirstRender(false);
    } else {
      // Set flag in localStorage to remember user has seen the animation
        ...
        {slides.map((slide, index) => (
          <SwiperSlide key={index}>
            {isFirstRender ? (
              ...
              /* Regular non-animated version for subsequent views */
              <div
                className={`
                  ${slide.bgColor}
                  rounded-3xl
              ...
              >
                ...
                <div className="relative z-10 text-white w-full max-w-md">
                  ...
                  {/* Description */}
                  <p className="text-white/90 text-base mb-6 max-w-[90%] mx-auto opacity-90">
                    {slide.description}
                  </p>
